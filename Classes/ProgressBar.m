...
- (void)setMode:(ProgressBarMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == ProgressBarModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(MBRoundProgressView1 *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == ProgressBarModeDeterminate) {
        self.indicator = [[MBRoundProgressView1 alloc] init];
    }
    else if (mode == ProgressBarModeCustomView && self.customView != nil){
...
+ (ProgressBar *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	ProgressBar *hud = [[ProgressBar alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return hud;
}
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[ProgressBar class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		ProgressBar *HUD = (ProgressBar *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"ProgressBarViewIsNillException" 
					format:@"The view used in the ProgressBar initializer is nil."];
	}
	//DLog(@"Y : %f",view.bounds.origin.y);
	//DLog(@"Y : %f",view.bounds.size.height);
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = ProgressBarAnimationFade;
        self.mode = ProgressBarModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == ProgressBarAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == ProgressBarAnimationZoom) {
